...
export default async function main(uiBuilder: UIBuilder, { t }: UseTranslationResponse<'translation', undefined>) {
  ...
  uiBuilder.form(
    ...
    async ({ values }) => {
      ...
      // 检查两个输出字段是否是同一个字段
      if (outputFieldDistance && outputFieldDuration && outputFieldDistance.id === outputFieldDuration.id) {
      ...
      latitudeRecords.forEach((latRecord, index) => {
        if (!longitudeRecords[index]) {
          ...
          uiBuilder.message.error(t('longitude_record_missing_error', { recordId: latRecord.record_id }));
          throw new Error(t('longitude_record_missing_error', { recordId: latRecord.record_id }));
        }
      });
      ...
      for (let index = 0; index < latitudeRecords.length; index++) {
        ...
        const [lon2, lat2] = longitudeRecords[index].value.location.split(',').map(Number);

        const { distance, duration } = await calculateDistance(
          lon1, lat1, lon2, lat2, distanceType, "6e1abfcb4d7681ab33ec051c0a25dfda",
          (errorMsg) => {
            uiBuilder.hideLoading();
...
async function calculateDistance(lon1, lat1, lon2, lat2, distanceType, apiKey, errorCallback) {
